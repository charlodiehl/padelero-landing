    ...
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${sc ? 'bg-background/85 backdrop-blur-2xl border-b border-foreground/5 shadow-lg shadow-scrim/40' : ''}`}>
      <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <Link href="/pro" className="flex items-center gap-1.5 text-sm font-bold px-4 py-1.5 rounded-lg transition-all hover:bg-primary/10" style={{ color: GREEN }}>
            <Sparkles size={14} /> Padelero Pro
          </Link>
          <Link href="/nosotros" className="flex items-center gap-1.5 text-sm font-bold px-4 py-1.5 rounded-lg bg-primary/10" style={{ color: GREEN }}>
            <Rocket size={14} /> Nosotros
          </Link>
        </div>
        <div className="hidden md:flex items-center gap-3">
          <Link href={`${APP}/login`} className="text-muted-foreground hover:text-foreground text-sm px-3 py-1.5 transition-colors">Ingresar</Link>
          <Link href={`${APP}/register`} className="bg-primary text-primary-foreground text-sm font-bold px-5 py-2 rounded-xl hover:bg-primary-hover transition-all hover:scale-105 shadow-md shadow-primary/20">
            Empezar gratis
          </Link>
        </div>
...
function FotoCarlos() {
  ...
  if (err) {
    return <div className="w-full h-full bg-gradient-to-br from-surface-2 to-card" />;
  }
  return (
...
function Eyebrow({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-block text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full border border-foreground/10 bg-foreground/[0.03] text-foreground-subtle">
      {children}
    </span>
  );
}
    ...
    <main className="bg-background text-foreground min-h-screen">
      ...
      <section className="relative isolate pt-32 pb-20 px-5 overflow-hidden">
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute inset-0 opacity-60"
            style={{ background: `radial-gradient(60% 50% at 50% 0%, hsl(var(--primary) / 0.094), transparent 70%)` }} />
        </div>
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          ...
          <Reveal delay={160}>
            <p className="mt-6 text-lg md:text-xl text-foreground-subtle leading-relaxed">
              Construir <strong className="text-foreground">la mejor aplicación de pádel de Argentina</strong> —
      ...
      <section className="px-5 py-16 border-t border-foreground/5">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-8">
            {FEATURES.map((t) => (
              <Reveal key={t} className="rounded-2xl border border-foreground/10 bg-foreground/[0.03] p-4">
      ...
      <section className="px-5 py-16 border-t border-foreground/5">
        <div className="max-w-5xl mx-auto grid md:grid-cols-[280px_1fr] gap-8 md:gap-12 items-start">
          <Reveal>
            <div className="rounded-3xl overflow-hidden border border-foreground/10 bg-foreground/[0.03] aspect-square">
              <FotoCarlos />
            </div>
          ...
          <Reveal delay={120}>
            ...
            <div className="space-y-4 text-foreground-subtle leading-relaxed">
              ...
              <p>
                En un viaje a <strong className="text-foreground">España</strong> me crucé con <strong className="text-foreground">Playtomic</strong>:
                la super-app que ordenó todo el pádel en Europa. Lo vi y pensé: <em>“esto en Argentina no existe… todavía”.</em>
              </p>
              <p>
                Así nació Padelero, con una apuesta distinta: hacerlo <strong className="text-foreground">solo</strong>,
          ...
          <div className="grid md:grid-cols-2 gap-4 mt-8">
            <Reveal className="rounded-2xl border border-foreground/10 bg-foreground/[0.03] p-6">
              ...
              <p className="text-foreground-subtle leading-relaxed text-sm">
                ...
                un equipo de <strong className="text-foreground">+200 personas</strong>.
              </p>
            </Reveal>
            <Reveal delay={120} className="rounded-2xl border-2 p-6 bg-primary/[0.04]" style={{ borderColor: `hsl(var(--primary) / 0.333)` }}>
              ...
              <p className="text-foreground-subtle leading-relaxed text-sm">
                ...
                respondemos: <em className="text-foreground">¿cuánto puede construir hoy un solo founder?</em>
              </p>
            </Reveal>
          </div>
        </div>
      ...
      <section className="px-5 py-16 border-t border-foreground/5">
        <div className="max-w-5xl mx-auto">
          ...
          <Reveal className="flex flex-col items-center">
            <div className="rounded-2xl border-2 px-6 py-4 bg-primary/[0.06] flex items-center gap-3" style={{ borderColor: `hsl(var(--primary) / 0.4)` }}>
            ...
            <div className="text-[11px] uppercase tracking-widest text-muted-foreground/70 mb-6">Coordina a</div>
          </Reveal>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {AGENTES.map((a, i) => (
              <Reveal key={a.name} delay={i * 60}
                className="rounded-2xl border border-foreground/10 bg-foreground/[0.03] p-4 border-l-4"
                style={{ borderLeftColor: a.color }}>
                <div className="font-bold text-sm" style={{ color: a.color }}>{a.name}</div>
                <div className="text-[12px] text-muted-foreground leading-snug mt-0.5">{a.role}</div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="px-5 py-20 border-t border-foreground/5">
        <div className="max-w-2xl mx-auto text-center">
          <Reveal>
            ...
            <p className="mt-5 text-foreground-subtle text-lg leading-relaxed">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <Link href={`${APP}/register`}
                className="inline-flex items-center justify-center border border-foreground/15 text-foreground font-bold px-6 py-3.5 rounded-xl hover:bg-foreground/5 transition-all">
                Probar Padelero
              </Link>
            </div>
          </Reveal>
      ...
      <footer className="border-t border-foreground/5 px-5 py-10">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-5 text-sm text-muted-foreground">
            ...
            <Link href="/clubes" className="hover:text-foreground transition-colors">Clubes</Link>
          ...
          <p className="text-xs text-muted-foreground/50">© {2026} Padelero</p>
        </div>
      </footer>
    </main>
  );
