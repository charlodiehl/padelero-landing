...
import {
  Sparkles, ArrowRight, Menu, X, Trophy, Home,
  Rocket, Globe, Users, Brain, Code2, Search, CheckCircle2,
  BarChart3, Megaphone, MessageCircle, Compass, MapPin, TrendingUp,
  type LucideIcon,
} from 'lucide-react';
...
const AGENTES: { icon: LucideIcon; color: string; name: string; role: string }[] = [
  { icon: Compass,       color: '#a78bfa', name: 'Product Manager', role: 'Convierte ideas crudas en specs implementables.' },
  { icon: Code2,         color: '#60a5fa', name: 'Engineer',        role: 'Escribe el código, agrega tests y abre los PRs.' },
  { icon: Search,        color: '#f59e0b', name: 'Reviewer',        role: 'Code review: bugs, regresiones, race conditions.' },
  { icon: CheckCircle2,  color: '#34d399', name: 'QA',              role: 'Prueba cada feature end-to-end, en producción.' },
  { icon: BarChart3,     color: '#22d3ee', name: 'Data Analyst',    role: 'Métricas, retención, GMV y reportes claros.' },
  { icon: Megaphone,     color: '#fb7185', name: 'Marketing',       role: 'Copys, campañas y contenido con voz de marca.' },
  { icon: MessageCircle, color: GREEN,     name: 'Support',         role: 'Responde a los clubes con el contexto del repo.' },
];
    ...
    <main className="bg-black text-white min-h-screen">
      ...
      <section className="relative pt-32 pb-20 px-5 overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center">
          <Reveal>
            <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full border border-white/10 bg-white/[0.03] text-zinc-300">
              <Sparkles size={13} style={{ color: GREEN }} /> La oportunidad
            </span>
          </Reveal>
          <Reveal delay={80}>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-8">
            {[
              ['🎾', 'Reservas y turnos'],
              ['🏆', 'Torneos y ranking'],
              ['🤝', 'Matchmaking y comunidad'],
              ['🤖', 'Agentes Pro para clubes'],
            ].map(([e, t]) => (
              <Reveal key={t} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-center">
        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-[280px_1fr] gap-8 md:gap-12 items-start">
          <Reveal>
            <div className="relative">
              <div className="rounded-3xl overflow-hidden border border-white/10 bg-white/[0.03] aspect-square">
                {/* Guardá la foto en /public/equipo/carlos.jpg */}
                <Image
          ...
          <Reveal delay={120}>
            ...
            <div className="space-y-4 text-zinc-300 leading-relaxed">
              ...
              <p>
                En un viaje a <span className="inline-flex items-center gap-1 text-white font-semibold"><MapPin size={14} style={{ color: GREEN }} /> España</span> me
                crucé con <strong className="text-white">Playtomic</strong>: la super-app que ordenó todo el pádel
                en Europa. Lo vi y pensé: <em>“esto en Argentina no existe… todavía”.</em>
              </p>
              <p>
      ...
      <section className="px-5 py-16 border-t border-white/5">
        <div className="max-w-4xl mx-auto">
          <Reveal>
            <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-zinc-400">
              <Globe size={14} style={{ color: GREEN }} /> El mercado
            </span>
            <h2 className="text-2xl md:text-3xl font-black mt-3 mb-4">Una categoría enorme, sin dueño local</h2>
            <p className="text-zinc-300 text-lg leading-relaxed">
          ...
          <div className="grid md:grid-cols-2 gap-4 mt-8">
            <Reveal className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
              <div className="flex items-center gap-2 text-zinc-400 text-sm font-bold uppercase tracking-wider mb-3">
                <Users size={16} /> La referencia
              </div>
            ...
            <Reveal delay={120} className="rounded-2xl border-2 p-6 bg-[#C8F542]/[0.04]" style={{ borderColor: `${GREEN}55` }}>
              <div className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider mb-3" style={{ color: GREEN }}>
                <Rocket size={16} /> El experimento
              </div>
      ...
      <section className="px-5 py-16 border-t border-white/5">
        <div className="max-w-4xl mx-auto text-center">
          <Reveal>
            <Brain size={32} style={{ color: GREEN }} className="mx-auto mb-4" />
          ...
          <Reveal className="flex flex-col items-center">
            <div className="rounded-2xl border-2 px-6 py-4 bg-[#C8F542]/[0.06] flex items-center gap-3" style={{ borderColor: `${GREEN}66` }}>
              <div className="h-12 w-12 rounded-full overflow-hidden border-2 shrink-0" style={{ borderColor: GREEN }}>
                <Image src="/equipo/carlos.jpg" alt="Carlos Diehl" width={96} height={96} className="w-full h-full object-cover" />
              </div>
            ...
            {AGENTES.map((a, i) => {
              ...
              return (
                <Reveal key={a.name} delay={i * 60}
                  className="rounded-2xl border-2 p-4 bg-white/[0.03] transition-all hover:bg-white/[0.06]"
                  style={{ borderColor: `${a.color}40` }}>
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-xl grid place-items-center shrink-0"
                      style={{ backgroundColor: `${a.color}22`, border: `2px solid ${a.color}50` }}>
                      <Icon className="h-5 w-5" style={{ color: a.color }} aria-hidden />
                    </div>
                    <div>
                      <div className="font-bold text-sm">{a.name}</div>
                      <div className="text-[11px] text-zinc-400 leading-tight">{a.role}</div>
                    </div>
                  </div>
                </Reveal>
              );
      ...
      <section className="px-5 py-16 border-t border-white/5">
        <div className="max-w-4xl mx-auto">
          <Reveal>
            <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-zinc-400">
              <TrendingUp size={14} style={{ color: GREEN }} /> Track record
            </span>
            <h2 className="text-2xl md:text-3xl font-black mt-3 mb-8">Lo que pasó en 3 meses</h2>
          </Reveal>
          ...
          <Reveal delay={120}>
            <div className="mt-8 rounded-2xl border-2 p-6 flex items-center gap-4" style={{ borderColor: `${GREEN}55`, background: `${GREEN}0A` }}>
              <Rocket size={28} style={{ color: GREEN }} className="shrink-0" />
              <div>
          ...
          <Reveal>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
              <Link href={WA} target="_blank"
                className="inline-flex items-center justify-center gap-2 bg-[#C8F542] text-black font-black px-6 py-3.5 rounded-xl hover:bg-[#d4ff4a] transition-all hover:scale-105">
                <MessageCircle size={18} /> Hablemos
              </Link>
              <Link href={`${APP}/register`}
                className="inline-flex items-center justify-center gap-2 border border-white/15 text-white font-bold px-6 py-3.5 rounded-xl hover:bg-white/5 transition-all">
                Probar Padelero <ArrowRight size={18} />
              </Link>
            </div>
          </Reveal>
        </div>
